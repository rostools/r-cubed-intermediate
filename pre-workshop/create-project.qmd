...
2. If you are not already in the `LearnR3` project, open it up by either
   clicking the `LearnR3.Rproj` file in the `LearnR3/` folder or by
   using the "File -\> Open Project" menu. Run the function below in the
   Console when RStudio is in the `LearnR3` project, which will create a
   new file called `learning.qmd` in the `docs/` folder.
