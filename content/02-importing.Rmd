...
with `library(vroom)` as well as `library(here)`. Since we'll also be using
the tidyverse package, let's also add `library(tidyverse)`. 
...
want to load other packages. After adding this code chunk, create a new code chunk
(`Ctrl-Alt-I`) right below it.
...
**Please stop reading now and we'll go over this together**.
...
it found an empty column with no column name and is letting you know it had
to rename it. Even though we excluded it with `col_select = -1`,
...
possible functions to use. We want to use the snakecase one, so scroll down and
...
Now we have another warning though. It's probably because the names are repaired
to snake case before the `col_types` are used. Let's try repeating the `vroom()`
to `spec()` to `vroom()` workflow we just used to see if that fixes the problem.
...
Remove the `...1` line. Add the `.name_repair` argument
...
`col_types()`. Don't forget to remove the `...1 = col_skip()` (or `NA = col_skip()`)
line from the `cols()` function.
...
- Use `col_types =` and `col_select = ` arguments in `vroom()` to import only
the data you need
- Fix column names to match a style using `.name_repair =` argument in `vroom()`
- For very large datasets, importing in the first 100-1000 rows using `n_max =`
