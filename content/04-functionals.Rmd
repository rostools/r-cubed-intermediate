...
in a loop
- Can use functions as input to other functions (called a [functional]), rather than having to create your own loop to do it
...
With `map()`, we'd do this a bit differently. purrr allows us to create anonymous functions (functions that are used once and disappear after usage) to extend its capabilities. Anonymous functions are created by writing `function(x)` followed by the function definition inside of `map()`. Using `map()` with an anonymous function allows us to do more things to the input vector (e.g. `1:5`). Here is an example:

```{r}
map(1:5, function(x) paste(x, "seconds have passed"))
```

As this is quite verbose, purr supports the use of a syntax shortcut to write anonymous functions. This shortcut is using `~` to start the function and `.x` as the replacement for the vector item. `.x` is used instead of `x` in order to avoid potential name collisions in functions where `x` is an function argument (for example in `aes()`, where `x` can be used to define the x-axis mapping for a graph). Here is the same example as above, now using the `~` shortcut:
...
and harder to write and read compared to these other tools. **Ok, stop reading and we'll go over this again before continuing with the coding.**
...
```{r eval=FALSE}
# Create list to contain answer/output
output <- vector("list", length = length(numbers))

# For loop
for(itterator in seq_along(numbers)) {
    output[[itterator]] <- over_five(numbers[[itterator]])
}
```

A loop allows us to do exactly the same, however, with more precise and shorter code. Additionally, a for loop can handle a vector of any length.

Still, writing loops for simple operations does not equate to functional programming. In this case, replacing the for loop with a the functional `map()`, would make the code more precise, and akin to functional programming:
...
Our code is now more functional, however, there is still room for improvement. For example, the `over_five()` function is rather specific and we are very unlikely to use this function anywhere else in our code. Hence, it would be a good idea to remove and replace `over_five()` with a anonymous function (with or without using the `~` shortcut):
...
Lastly, our code does not incorporate any form for vectorization. The for loop we created, runs code sequentially, where only one value is evaluated at a time. Vectorization would allow us to perform this check in parallel. To incorporate vectorization we could either create a vectorized function ourselves (this is quite difficult and complex) or find an existing function that is already vectorized. In this case, the function `if_else()` is already vectorised, so we'll use that:
...
## Exercise: Brainstorm and discuss ways to incorporate functionals into your work

Time: 10 min

As a group, brainstorm and discuss as many ways as you can for how you might
incorporate functionals like `map()` into your own work. Afterwards, all groups
will briefly share some of what they thought of before we move on to the next exercise.
...
1. Repeat the steps you've taken previously to create a new function:
    ...
    `import_function`. 
    - Within in the code, replace and re-write `"user_info.csv"` with
    `file_pattern` and `import_user_info` with `import_function`.
...
with the `where()` function, you can select different variables. The `where()` function is a `tidyselect` helper, a set of functions that make it easier to select variables. Some additional helper functions include:

```{r echo=FALSE}
tibble::tribble(
    ~'What it selects', ~Example, ~Function,
    'Select variables where a function returns TRUE', 'Select only variables that is of character type (is.character)', 'where()',
    'Select all variables', 'Select all variables in "user_info_df"', 'everything()',
    'Select variables that contains matching string', 'Select variables that contains the string "user_info"', 'contains()',
    'Select variables that ends with suffix', 'Select all CSV files with ".csv"', 'ends_with()'
    ) %>% 
    knitr::kable(caption = "Common tidyselect helper functions and some of their use cases.")
```

Lets select columns that are numeric:
