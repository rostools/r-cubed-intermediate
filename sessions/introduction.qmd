...
others and ourselves to reproduce the work. Below are sections for the
different workflows we will be using in the workshop.
...
already and are fairly confident that it works as you want it to.

So you'll use Quarto (`doc/learning.qmd`) to write and test out code,
...
and keeping finalized, tested code. This is also a very common workflow
in R and fits with other workflows we will encounter in the [advanced
workshop](https://r-cubed-advanced.rostools.org). Then, within the
Quarto document we can `source()` the `R/functions.R` script so we have
access to the stable and tested code. This workflow is represented below
in @fig-quarto-to-functions.
...
## Saving the current state of the project in Git
...
Now that we've gone over the bigger picture and some workflows we will
use in the workshop, let's get our project ready for the next session.
While you did that work in the pre-workshop tasks downloading the data
and unzipping it, we need to save these changes to the Git history. Open
the Git interface with either the Git icon at the top near the menu bar
or with {{< var keybind.git >}}. Then select all the current changes by
clicking the checkbox beside the files and stage them. Then we write a
commit message in the text box on the right, something like
`Setting up the project`. Click the "Commit" button, then close the Git
interface.
