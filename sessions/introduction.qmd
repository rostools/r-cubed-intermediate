...
Firstly, what we ultimately want to do is process the MMASH data so that
we have a single dataset to work with for later (hypothetical) analyses.
...
order to achieve our aim of making a single data and to make things
reproducible.

Both the folder and file structures below as well as the Figure
@fig-diagram-overview-0 show exactly what we will be doing and how it
will look like at the file level (not at the R code level). Hopefully
with this overview, you can better understand where we are and where we
want to get to. A comment on naming: whenever folder names are given,
...
conventions in R, like having a `DESCRIPTION` file, keeping raw data in
the `data-raw/` folder, and keeping R scripts in the `R/`. We also want
to keep things structured to make it easier for others and ourselves to
reproducible the work.

Our workflow will generally look like Figure @fig-diagram-overview-0,
with each block representing one or two sessions. We've already done a
bit of the first block, *"Download raw data"*.

![Overview of the workflow we will be using and
covering.](../images/diagram-overview-0.svg){#fig-diagram-overview-0}
...
-   Use R Markdown (`doc/lesson.Rmd`) to write and test out code,
    convert it into a function, test it, and then move it into
    ...
    -   It's easier to quickly test code out and make sure it works
        ...
        structure. Think of using the R Markdown file as a sandbox to
        test out and play with code, without fear of messing things up.
    -   We also test code out in the R Markdown because it's easier from
        a teaching perspective to interweave text and comments with code
        as we do the code-alongs and because it forces you to practice
        working within R Markdown documents, which are key components to
...
-   Use a combination of restarting R (`Ctrl-Shift-F10` or "Session -\>
    Restart R") and using `source()` (`Ctrl-Shift-S` while in
    ...
    -   We restart R because it is the only sure way there is to
        ...
        clean plate.
-   Replace the old code in the R Markdown document (`doc/lesson.Rmd`)
...
-   Finish building functions that prepare the dataset, use them in the
    ...
    -   Like with the `R/functions.R` script, we move the finalized data
        processing code over from the R Markdown into the
...
in the commit textbox saying `Added initial files`. Now we're ready for
the next session!
