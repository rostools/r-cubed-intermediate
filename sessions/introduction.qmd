...
In our case during the workshop, we ultimately want to process the MMASH
...
reproducible. So everything we'll do throughout the workshop is done in
...
-   Make an R script (`data-raw/mmash.R`) to download the dataset
    (`data-raw/mmash/`), which you already did in the pre-workshop tasks.
...
-   Whenever we complete a task, we *add and commit* those file changes
    ...
    -   We use Git because it is the best way of keeping track of what
        ...
        documentation).
    -   **Note**: While we covered GitHub in the introductory workshop, we
        can't assume everyone will have taken that workshop. Because of
        that, we won't be using GitHub in this workshop.
::::
...
that work in the pre-workshop tasks downloading the data and unzipping it,
...
will make the rest of the workshop a bit easier. We'll include some code
...
<!-- TODO: Remove this after workshop ends, since the same info is in pre-workshop now -->
...
based on code you select and highlight. Throughout the workshop, we will
...
chances to use it as we go through the workshop. The thing to note is that
