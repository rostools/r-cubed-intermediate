...
![Left joining in `{dplyr}`. Notice how the last row in the blue data (the row with `d` in column `A`) is not included in the outputted data on the right. Modified from the Posit `{dplyr}` [cheat sheet](https://opensource.posit.co/resources/cheatsheets/data-transformation/data-transformation.pdf).
...
![Right joining in `{dplyr}`. Notice how the last row in the green data (the row with `c` in column `A`) is not included in the outputted data on the right. Modified from the Posit `{dplyr}` [cheat sheet](https://opensource.posit.co/resources/cheatsheets/data-transformation/data-transformation.pdf).
Image license is CC-BY-SA.](/images/right-join.png){#fig-right-join
width="90%" fig-alt="A diagram showing two data frames, one green and
one blue, with three columns each. Each data frame has one different
column and one different row. The output data frame shows that all
...
![Full joining in `{dplyr}`. Notice how all rows and columns are included in the outputted data on the right, and that some missingness is introduced because those values don't exist when the data are combined in this way. Modified from the Posit `{dplyr}` [cheat sheet](https://opensource.posit.co/resources/cheatsheets/data-transformation/data-transformation.pdf).
...
::: callout-caution
:construction_worker: This part is being updated. Once ready, it will be
shown here.
:::
...
<!-- TODO: Fix the `r` situation -->

Notice the number of rows in each dataset. The `participant_details`
dataset has `nrow(participant_details)` rows and the `cgm_data` dataset
has `nrow(cgm_data)` rows. When we `left_join()` them, there should no
more than `nrow(participant_details) + nrow(cgm_data)` rows (`cgm_data`
plus `participant_details`) in the output. There will likely be less
because, remember, we have only imported the first 100 rows of each
dataset. Let's join them with `left_join()`.
...
`nrow(left_join(participant_details, cgm_data))`, which is slightly less
than we expected but it also isn't surprising. Some rows in `cgm_data`
got dropped because we haven't yet imported all the data.
...
than `nrow(participant_details) + nrow(cgm_data)` rows though.
...
means there should be exactly `nrow(cgm_data)` rows in the output.
...
`nrow(participant_details)` rows. Let's try `full_join()` to see what
happens.
...
`nrow(full_join(participant_details, cgm_data))`, which is still less
