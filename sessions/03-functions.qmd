...
```{r create-add-function, purl=TRUE}
...
`doc/learning.qmd` file and into the `R/` folder. We do this for a few reasons:
...
`import_user_info()` function we created in `doc/learning.qmd`, including the
...
"Session -> Restart R" menu item. Move back into the `doc/learning.qmd`
...
```{r explicit-function-calls, purl=TRUE}
import_user_info <- function(file_path) {
...
```{r solution-update-other-functions, results='hide', solution=TRUE, purl=TRUE}
#' Import the MMASH saliva dataset.
#'
#' @param file_path Path to the user saliva data file.
#'
