...
@fig-diagram-overview-3.

![Section of the overall workflow we will be
covering.](/images/diagram-overview-3.svg){#fig-diagram-overview-3}
...
![A function using vectorization. Modified from the [RStudio purrr
...
vector. Modified from the [RStudio purrr
...
4.  After it works, cut and paste the function into the `R/functions.R`
    ...
    **Console**.
5.  Once done, **add** the changes you've made and **commit** them to
...
helper functions are listed in @tab-tidyselect-helpers.

```{r tab-tidyselect-helpers}
#| echo: false
...
When you look in `?across`, there are two main arguments and two
optional ones:
...
2.  `.fns`: The function to use on the `.cols`.
    ...
    -   A named list with bare functions
        ...
        `cortisol_norm_average`).
    -   A function passed with `~` and `.x`, like in `map()`. For
        instance, `across(c(age, weight), ~ mean(.x, na.rm = TRUE))` is
        used to say "put age and weight, one after the other, in place
        of where `.x` is located" to calculate the mean for age and the
        mean for weight.
3.  `...` argument: Arguments to give to the functions in `.fns`. For
    instance, `across(age, mean, na.rm = TRUE)` passes the argument to
    remove missingness `na.rm` into the `mean()` function.
...
Before continuing, let's knit the Quarto / R Markdown document with
...
8.  Knit the `doc/learning.qmd` document with {{< var keybind.render >}}
