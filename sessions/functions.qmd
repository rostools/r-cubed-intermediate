...
some simple cleaning to it. But we have nearly 3000 CSV files to read
in. So even with our simple code, it isn't going to be very efficient if
we just copy and paste that code to read in all those files. But one
powerful concept to use in R is: "if you can do it for one, you can do
it for all". This session is about bundling our code into functions to
"do it for one" that will make it easier to later "do it for all", while
using less code.
...
1. Have the function's input (the arguments) be generic enough to take
   objects of the same "type" but that have different values (e.g. a
   number type, but any number is allowed). In our case, we have a
   function that takes a path "type". A path is a very general input, so
...
1. In the `read()` function, add a new argument called `max_rows` after
   ...
   `max_rows` because that's a more descriptive name for it than
   `n_max`.
