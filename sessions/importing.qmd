...
untar(
  ...
  exdir = fs::dir_ls(project_path, all = TRUE, recurse = TRUE)
)
...
data*" in Figure @fig-diagram-overview-1. You already downloaded and
unzipped the raw data during the [pre-course](/preamble/pre-course.qmd)
tasks and here we'll continue the process by now importing it into R.
