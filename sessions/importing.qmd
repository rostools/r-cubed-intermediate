...
for the workshop in the Quarto document! :grin: So if you want to come back
and look at your notes later, this would be the place to put them.
:::
...
Before we do that though, let's tell Git to ignore some other files, like
the `.html` files as well as the `_files/` folder that Quarto automatically creates when making an HTML document.
In the **Console**, type out the following code:
...
Then, open the Git interface with either the Git icon at the top near the menu
bar or with {{< var keybind.commit >}}. In the interface, select all the
