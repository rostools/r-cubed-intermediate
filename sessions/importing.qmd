...
**Save this token in your password manager**. This is the token you will
use every time you open up RStudio and interact with GitHub through R.
You do **not** need to create a new token for each R project or package
you make, you only need to create one after your current token expires
...
1. Create a new header at the bottom of the `docs/learning.qmd` file and
   call it `## Exercise: Import 101's sleep data`.
...
3. Copy and paste the code template above into the new code chunk. Begin
   ...
   information.
   - Give the correct file path in `here()` to 101's sleep data in the
     `sleep/` folder.
   ...
   - Import only first many rows with `n_max`. Instead of 100 like we
     did above, use another number of your choosing. Maybe fewer rows?
     Maybe a bit more?
...
- For very large datasets, importing in the first 100-1000 rows using
  `n_max =` to test that importing works fine before importing the full
  dataset.
...
- How tidy and clean is your data in your research? Do you have to do a
  lot of processing before you can analyse it?
