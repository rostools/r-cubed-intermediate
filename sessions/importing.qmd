...
6. Now try a different participant and a different dataset. This time,
   read in the `EDA.csv.gz` file for participant `6B` in the folder
   `6B_1587569373/` with and without `n_max = 100`.
7. Once you have the code working, run `{styler}` with
...
repetitive. If you wanted to read in all the data files (and the nurses'
stress data has a lot), you can imagine that this becomes quite tedious
and boring, not to mention error-prone and difficult to change (if you
decided to change how you've read the data). Next session we will learn
