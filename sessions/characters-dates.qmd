...
use to get the participant ID from the `file_path_id` column.
...
characters like `[` or `?` have special meanings. For instance,
`[aeiou]` is regex for "find one character in a string that is either a,
e, i, o, or u". The `[]` in this case means "find the character in
between the two brackets". We won't cover regex too much in this
workshop, but some great resources for learning them are the [R for Data
Science regex
...
| `[a-z]` | find one letter character between a and z |
| `[A-Z]` | find one letter character between A and Z |
| `[a-zA-Z]` | find one letter character between a and z or A and Z |
| `\\.` | find one dot character |
| `$` | find the end of the character object, e.g. `\\.$` means find a dot that is at the end of the character object |
| `^` | find the start of the character object, e.g. `^\\d` means find a digit at the start of the character object |
| `?` | maybe find the character before the `?`, e.g. `\\d?` means maybe find one digit |
| `*` | find the character before it, but maybe many times, e.g. `\\d*` means find zero or many digits one after the other |
| `+` | find the character one or more times before `{}`, e.g. `\\d+` means find one or more digits one after the other |
| `{n}` | find the same character `n` times before `{}`, e.g. `\\d{2}` means find two digits one after the other |
| `[:alpha:]` | find one letter character (a-z or A-Z) |
| `[:digit:]` | find one digit character (0-9) |
| `[:alnum:]` | find one alphanumeric character (a-z, A-Z, 0-9) |

::: callout-note
The `\` character is a special character in regex, but in R it means
"escape or use this as the character". For instance, `\"` in R means
"use the `"` character", like if you wanted to write a message like:
...
And think `"Analysis has "` is the first character object, that `nearly`
is an object, and the `" finished"` is the second character object.

But in regex, the `\` character is a special character that can mean
...
package to "extract a string" (a character object) by using the regex
`[:digit:]+\\.csv$` that we discussed from the activity above.
...
`str_remove()` function to remove the `.csv` from the character object.
So we can pipe our output into that function:
...
```{r solution-participant-id-function}
#| include: false
...
By wrapping `column` with `{{}}`, we are telling R that we are giving it
an unquoted variable and to give that variable to the `as_date()`
...
Quickly cover this before finishing the session.
:::

-   While very difficult to learn and use, regular expressions (regex or
    regexp) are incredibly powerful at processing character data. Some
    characters have special meanings like `[]`, `?`, `*`, and `+` that
    can be used to find patterns in character data.
-   Use the `{stringr}` package to work with character data as it has
    many functions to help use regex to process character data, like
