...
to the third block, "*Create final data*", in @fig-diagram-overview-4.

![Section of the overall workflow we will be
covering.](/images/diagram-overview-3.svg){#fig-diagram-overview-4}
...
-   After writing it and testing that the function works, move the
    function into `R/functions.R`.
-   Replace the code in the `doc/learning.qmd` file with the function
    ...
    works.
-   Knit / render the `doc/learning.qmd` file to make sure things remain
...
![Left joining in dplyr. Modified from the RStudio dplyr
...
![Right joining in dplyr. Modified from the RStudio dplyr
...
![Full joining in dplyr. Modified from the RStudio dplyr
...
only one output remains. Modified from the RStudio purrr
...
#| include: false
save.image(here::here("_temp/dplyr-join.RData"))
```
