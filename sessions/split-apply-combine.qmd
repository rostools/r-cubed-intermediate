...
commit** the changes so far with {{< var keybind.commit >}} and push
to GitHub.
...
Let's start with converting the summarising code into a function. The
way it is used should be something like the below, and should give
similar output:

{{< include /includes/solutions/_split-apply-combine-summarise-column-function.qmd >}}
...
1.  Assign `function()` to a new named function called
    `summarise_column` and include three arguments in `function()` with
    the names `data`, `column`, and `functions`.
2.  Put the code we just wrote into the body of the function. Make sure
    to `return()` the output at the end of the function. Replace the
    relevant variables with the arguments you just created (e.g.
    `cgm_data` to `data`).
3.  Remember to use `{{ }}` when you use the `column` argument within
    the function to work with `{tidyverse}`'s and R's non-standard
    evaluation.
4.  To create the Roxygen documentation with {{< var keybind.roxygen >}}
    and fill it out. You can write whatever you like.
5.  Add `tidyselect` as a package dependency by using
    `usethis::use_package()` in the **Console**.
6.  Explicitly link the functions you are using in this new function to
    their package by using `::` (e.g. `dplyr::` and `tidyselect::`).
    ...
    package name.
7.  Test that the function works by using the code shown above.
8.  After creating the function and testing that it works, move (**cut**
    and paste) the function into `R/functions.R`.
9.  Run `{styler}` in both the `R/functions.R` file and
    `docs/learning.qmd` with {{< var keybind.style >}}.
10. Add and commit the changes to the Git history with
    {{< var keybind.commit >}}. Afterwards, push to GitHub.

In the next exercise, you will use this function in the recently created
`clean_cgm()` and the `clean_sleep()` function you will create.

::: {.callout-note collapse="true"}
## :teacher: Teacher note

Go over the solution to make sure everyone is on the same page, as this
is one of the few exercises that later work depends on. Then move to the
next exercise.
:::
