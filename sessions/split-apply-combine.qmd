...
edited. Once the session is finished being edited, the content will be
added here.
:::
...
In R, one way to use the split-apply-combine technique is by just using
`summarise()` function along with the `.by` argument.
...
At the bottom of the `docs/learning.qmd` file, create a new header
called `## Summarising by groups` and create a code chunk below that
with {{< var keybind.chunk >}}. First, since we won't be using the
`device_timestamp` column, let's remove it.
...
Then, let's continue and pipe `|>` to `summarise()`. We'll start with
summarising the mean and standard deviation of the glucose values by
using `across()`. use `across()` to calculate the mean and standard
deviation of the glucose values.
...
Notice how it summarised all the values in the dataset, so we end up
with only one row in total. But let's now group by the `id`, `date`, and
`hour` columns so that we get the mean and standard deviation for each
participant, for each date, and for each hour. We will use the `.by`
argument in `summarise()` to do this. In order to use the `.by` argument
with multiple columns, we need to use `c()` to combine the columns
together. So, we will add `.by = c(id, date, hour)` to the end of the
`summarise()` function.
...
cgm_data |>
  select(-device_timestamp) |>
  summarise(
    across(glucose, list(mean = mean, sd = sd)),
    .by = c(id, date, hour)
  )
```
...
1. The sleep data has an extra `sleep_type` column that we have to group
   by. We'd rather not have to include an argument in our new function
   just for indicating the grouping. It would be nice if we could group
   by all columns except for the column we want to summarise.
...
4. Put `data` at the top of the code and pipe it into the cleaning code.
...
- Build functions to be able to chain together and then convert those
  chained functions into general functions. That way you can create more
  "higher-level" functions to help keep your code organised, readable,
  and maintainable.
...
- From what you have learned so far from this session, how might you use
  this in your work?
