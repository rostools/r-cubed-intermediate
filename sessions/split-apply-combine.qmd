...
2.  Put the code we just wrote into the body of the function. Make sure
    ...
    `cgm_data` to `data`).
3.  To create the Roxygen documentation with {{< var keybind.roxygen >}}
    and fill it out. You can write whatever you like.
4.  Add `tidyselect` as a package dependency by using
    `usethis::use_package()` in the **Console**.
5.  Explicitly link the functions you are using in this new function to
    ...
    package name.
6.  Test that the function works by using the code shown above.
7.  After creating the function and testing that it works, move (**cut**
    and paste) the function into `R/functions.R`.
8.  Run `{styler}` in both the `R/functions.R` file and
...
As we prepare for the next session and take a break, get up and discuss
with your neighbour (or others) the questions:
