...
- Sometimes functions have their package name appended with `::` so that
  you run the code from the specific package, since we likely haven't
  loaded the package with `library()`. For instance, to install packages
  from GitHub using the `{pak}` package we use
