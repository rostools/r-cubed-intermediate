...
knitr::include_graphics(here::here("images/pivot-wider.svg"))
```
...
to find and fix problems in your code, there are two ways to do it:

- By typing in `browser()` at the start of your function and then re-running it
(either by manually running it or with `Ctrl-Shift-L` to `load_all()`).
This works for all functions in all R Project types.
...
While `browser()` works with non-Package Projects, we're going to cover and use
...
Activate it by running `load_all()` with `Ctrl-Shift-L`. The dot should now be
...
and `sd()`. This task will be challenging, so discuss and work with your
neighbour on completing this exercise.

- Work within the debugging environment to fix this problem
