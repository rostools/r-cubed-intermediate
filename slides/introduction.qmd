...
::: columns
::: {.column width="50%"}
-   :heavy_check_mark: Select a table name from the basket and go to the
    table name
...
# :shrug:...Because code sharing is almost non-existent in science {.center}
...
::: aside
1.  Also from an American Statistical Association
...
## Practice using stickies: Does your `data-raw/` look as this? {.center}
...
## :raising_hand: Who has seen or worked with "true" data analysis pipelines (e.g. run a single command and *everything* gets re-done)?
...
## :talking: For those that have done or are doing data processing, does it take up a lot of time? How much? {.center}
