...
previously that imported the `saliva.csv`, `RR.csv`, and `Actigraph.csv` data.
...
uses the argument `regexp` (short for [regular expression]) to find the pattern,
and `recurse` to look in all subfolders.
...
## Exercise: Use functionals for importing the other datasets

Time: 20 min

Inside `doc/lesson.Rmd`, use the `map_dfr()` functional on each of the
`saliva.csv`, `RR.csv`, and `Actigraph.csv` datasets.

1. Create a new header `## Exercise: Map on the other datasets` at the bottom of
the document.
1. Create a new code chunk below it.
1. Using `dir_ls()` and `map_dfr()`, importing in all the individual datasets
to be one each for the saliva, RR, and Actigraph data.
    - You should be using three instances of `dir_ls()` and `map_dfr()`
    for each of the three datasets.
    - Don't forget to add the `.id` argument to `map_dfr()`
1. Once done, add and commit what you've done.
