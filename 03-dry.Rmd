...
your other code too! Now that we have a working function, let's add and commit
it to the Git history with the RStudio Git Interface.
...
converting it into a function, like we did above.
    ...
    - Make a meaningful name (e.g. `import_saliva_data`)
...
1. To prevent the R Markdown document from becoming incredibly long, with only
R code.
...
Because this project is set as a Package, we can use the `load_all()` keybinding
`Ctrl-Shift-L` in RStudio. But first, we need to make a new file to store our
functions. So, in the *Console*, type out and run:

```{r functions-script, eval=FALSE}
usethis::use_r("functions")
```

This creates a new file called `R/functions.R`. We will cut and paste the
...
not finding the `vroom()` function. Or you might not, if you put
`library(vroom)` in the code chunk called `setup`. If you did get an error,
that's because R doesn't know what the `vroom()` function is.
...
depend on it for your code. The informal way to show what packages you use
is when you use the `library()` function. But what if someone else looks over your
code? Do they have to search through all your files just to find all `library()`
functions you used and then install those packages individually? A better way
here is to formally indicate your package dependency so that installing
dependencies is easy! This is done through the `DESCRIPTION` file.
...
- package dependency
    - use_package
    - ::
        - We do this step at the end because it can be tedious always 
        typing out `package::`.
    
You may have noticed from others code that people do:
...
consequences that won't give any warning or error. Instead we use `packagename::`
to access the function from a package. We won't get into the reasons why because
it can quickly get quite technical.
...
programming is functional programming. So what is functional programming? It is a
form of programming that:
...
Figure \@ref(fig:image-vectorization) shows how a function

```{r image-vectorization, fig.cap="A function using vectorization. Modified from the [RStudio purrr cheatsheet][purrr-cheatsheet].", out.width="75%", echo=FALSE}
knitr::include_graphics(here::here("images/vectorization.svg"))
```

So in R, you have instead the function called `sum()` that takes the entire
vector of `values` and outputs the total sum, without needing to use a for loop.
...
will always output a list. Let's say we wanted to paste together the number with
the sentence "seconds have passed". Normally it would look like:
...
functionals, you generally should only use for loops as the very last choice you
have. That's because R has better, more expressive, and more powerful tools to
...
The first thing we have to do to use the `map()` functional is to get a list
of all the dataset files available to us. We turn back to use fs, which has
a function called `dir_ls()` that finds files of a certain pattern.
In our case, the pattern is `user_info.csv`. Let's go back into our
`doc/lesson.Rmd` file, create a new header called `## Using map`, and
create a code chunk below it. Then, go to the `setup` code chunk at the top
and add to fs and purrr packages to the list of other packages.
...
# Your output will look a bit different
user_info_files
```
...
What if you want to output a character vector instead? There is a function
called `map_chr()` for that. There are several others that give an output based 
on the ending of `map_`:

- `map_int()` outputs an integer.
- `map_dbl()` outputs a numeric value, called a "double" in programming.
- `map_dfr()` outputs a data frame, combining by row (`r`).
- `map_dfc()` outputs a data frame, combining by column (`c`).
...
## Exercise: Use functionals for the other datasets

Time: 20 min

Use the `map_dfr()` functional on each of the `saliva.csv`, `RR.csv`, and
`Actigraph.csv` datasets.
