...
where the `import_user_info` function is. This is the perfect chance to make a function
that you can use for other purposes and that is itself a functional (since it
takes a function as an input). So inside `doc/lesson.Rmd`, convert this bit of
code into a function that works to import the other three datasets.
...
1. Repeat the steps you've taken previously to create a new function:
    ...
    - Create two new arguments called `file_paths` (notice the `s`) and `import_function`
    - Create generic intermediate objects (instead of `user_info_file` and `user_info_df`).
    I'd recommend something like `data_files` and `combined_data`
