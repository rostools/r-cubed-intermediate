...
## Summarizing data through functionals
...
analyzing more?
    - Keep those columns as well as `file_path_id` and `day` by using `select()`.
5. Decide which summary measure(s) you think may be most interesting for you
(e.g. `median()`, `sd()`, `mean()`, `max()`, `min()`, `var()`).
6. Using `group_by()` of `file_path_id` and `day`, summarize the variables with the
