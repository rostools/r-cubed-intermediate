...
```{r solution-summary-extend, eval=FALSE, echo=FALSE}
...
mmash %>% 
    tidy_summarise_by_day(mean) 
```

## Making prettier output in R Markdown

`r details_for_instructors("
Can go over this quite quickly.
")`
...
can use as a guideline to completing the following exercises and group work.
...
an R Markdown document, then cut and paste into a data processing R script in the
`data-raw/` folder.
- Restart R and generate the `.rda` dataset in the `data/` folder by
sourcing the R script.
- Restart R, load the new dataset with `load()` and put it into an R Markdown 
document.
...
- Write R in code chunks in an R Markdown document to further
