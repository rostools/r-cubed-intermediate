...
extensively in many R packages, is the foundation of tidyverse packages,
...
```{r solution-what-is-pipe, echo=FALSE}
# There is no code for this exercise.
```
...
`extract_user_id(user_info_df)`), restart the R session, load everything with
`load_all()` (`Ctrl-Shift-L`), and run the new function.
