...
```{r drop-file-path-id, eval=FALSE}
user_info_df %>% 
    mutate(user_id = str_extract(file_path_id, "user_[0-9][0-9]?")) %>% 
    select(-file_path_id)
```

```{r drop-file-path-id-eval, echo=FALSE}
user_info_df %>% 
    mutate(file_path_id = gsub(".*\\/data-raw", "data-raw", file_path_id)) %>% 
    mutate(user_id = str_extract(file_path_id, "user_[0-9][0-9]?")) %>% 
    select(-file_path_id)
...
`imported_data`.
- Don't forget to output the code into an object and then adding `return()` at
the end or to include Roxygen documentation.
...
and transformation. In our case, 
...
One big reason to get comfortable and good at using `map()` is because you can
use parallel processing to speed up your analysis with the `future_` series of
...
plan(multisession)
# This can't be put into the function, has to be used outside of it.

# Add this to end so it returns to original
plan(sequential)
```

## Exercise: Add parallel processing to data-raw script.
