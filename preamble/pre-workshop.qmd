...
start the workshop with everything set up. 

TODO: Include?

For some of the tasks, you
may not understand why you need to do them, but you will likely
understand why once the workshop begins.


<!-- TODO: Convert this into an include/snippet? -->

These tasks could take between
**2-6 hrs to finish** depending on:

- Your current skill and knowledge level when it comes to computers, installing programs,
and previous experience in working with the material we will cover in this workshop.
- If you will be using a work laptop and if your institution has more restrictive policies on what you can install on your computer. You may need to contact your work IT sooner than later for help or to get permissions. From our experience, this can be the hardest part of these pre-workshop tasks since it can be out of your control.

So, we suggest planning at least a full day to complete
these pre-workshop tasks and to start them sooner than later.
...
1.  Read the learning objectives in @sec-pre-workshop-learning-objectives
    for the pre-workshop tasks (below).
...
8.  Download the data in @sec-download-data and save it
    to your computer. Run a check in this subsection to see that
    everything is as expected. You'll later need to paste this output
    into the survey.
9.  Read about the basic workshop details in @sec-workshop-details.
10. Read the syllabus in @sec-syllabus.
11. Read the [Code of
    Conduct](https://guides.rostools.org/conduct.html).
12. **Complete the pre-workshop
    [survey](https://docs.google.com/forms/d/e/1FAIpQLSe2gpbN9ExCXyeTfJLgu_poemF0iUtkcihnCunXl3YA4e1hHQ/viewform?usp=sf_link)**.
...
runs smoothly. As part of the preparation are some tasks for you to do
some pre-requisite reading and learning, to best prepare you and your fellow
learners for the workshop. So in the learning objectives for this section
will enable you to:
...
{{< include _extensions/rostools/r3-theme/includes/reading-website-r.md >}}

## Installing the latest programs {#sec-install-programs}

{{< include _extensions/rostools/r3-theme/includes/installing-programs-r.md >}}

## Installing the R packages {#sec-install-r-packages}

TODO: Convert this to includes, with params at the top.
...
Since Git has already been covered in the [Introduction workshop](https://r-cubed-intro.rostools.org), we won't
cover *learning it* during this workshop. However, since version control
is a fundamental component of any modern and reproducible data analysis workflow and
*should be used*, we **will be using it throughout the workshop**.

::: callout-important
If you have used or currently use Git, you can skip this section and move on to @sec-confirm-git-setup.
:::

Git itself isn't too difficult to use. But the concepts that Git uses are very difficult to learn and use.
That's because Git requires you to view your files and folders and computer *very differently* to how you may
normally be used to working with them. And because we will be using Git throughout the workshop, 
to ensure you have the smoothest experience, we want you to at least be aware of and have read a bit about
Git. So, from the introduction workshop, please read these sections:

1. [What is version control?](https://r-cubed-intro.rostools.org/sessions/version-control#sec-what-is-version-control)
2. [What is Git?](https://r-cubed-intro.rostools.org/sessions/version-control#sec-what-is-git)
3. [Basics of Git](https://r-cubed-intro.rostools.org/sessions/version-control#sec-basics-of-git)

After reading them, in your RStudio in the R Console, run this function that should
open up a pop-up window to type in your name and email:

``` {.r filename="Console"}
r3::setup_git_config()
```

For most of the workshop, we
...
section. Later on during the workshop, we might connect our projects
...
Since we will be using Git regularly throughout the workshop, please run
this check to ensure everything is setup correctly. In R Console while in RStudio,
run this function:
...
::: callout-note
While GitHub is a natural connection to
using Git, given the limited time available in the workshop, we will not be going over
how to use GitHub. If you want to learn about using GitHub, check out
this
[session](https://r-cubed-intro.rostools.org/sessions/github)
...
analysis workflow is to keep everything self-contained in a single location. In
...
issues with participants when they do the workshop.
:::
...
1.  Please read over the [Quarto
    ...
    of the introduction workshop. If you use Quarto already, you can skip
    this step.
...
Throughout the workshop, we will use this document as a sandbox to test
code out and then move the finished code to other files.

## Download the workshop data {#sec-download-data}

To best demonstrate the concepts in the workshop, we ideally should work
on a real dataset to apply what we're going to learn. So for this
workshop, we're going to use an openly licensed dataset on monitoring
sleep and activity ([DIME](https://zenodo.org/records/8268744)) [@Traka2023]. 

TODO: Add link to script that downloads the data.

If you want to see
Much of the same concepts and skills you will learn in this workshop were
used to process and prepare this dataset for you to use in this workshop.
So 
...
your computer. The goal at the end of the workshop is to create a pipeline
...
get is rarely all cleaned up and ready for you, and this workshop is about
learning more advanced tools to do the data wrangling. 
...
1.  Look through the Data Description to get familiar with the dataset
    ...
    Description throughout the workshop as well as in the exercises.
...
``` {.r filename="Console"}
fs::use_data_raw("dime", open = FALSE)
```
...
in the introductory R workshop.
...
through the workshop, you will see why this is important to consider.
...
```{r pre-workshop-state-of-script}
#| filename: "data-raw/mmash.R"
#| file: "../R/pre-workshop-script.R"
#| eval: false
```

```{r admin-pre-workshop-state-of-script}
#| include: false
fs::file_copy(
  here::here("R/pre-workshop-script.R"),
...
know. The final pre-workshop task is a survey that asks some questions on
