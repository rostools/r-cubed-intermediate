...
fix it if you restart R by going to `Sessions -> Restart R` and
re-running this code, it should work. If it still doesn't, try to
...
2.  Don't create the project on any shared drive (like `H:` or `E:`
    drives on Windows). Because these folders are remote locations on an
    external server, they can really slow things down when working with
    Git, R, and RStudio. Create the project on your actual computer,
    like the `C:` drive in Windows or `/Users/` on Mac.
...
#| include: false
tar(
  here::here("_temp", "pre-course.tar.gz"),
  fs::dir_ls(project_path, all = TRUE, recurse = TRUE)
)
```
