...
```{r diagram-overview, fig.cap="Overview of the workflow we will be using and covering.", echo=FALSE}
diagram_overview(0)
```
...
- We use a combination of restarting R (`Ctrl-Shift-F10` or "Session -> Restart
...
steps. First thing, though. Since we did all that work in the pre-course tasks
downloading the data and unzipping it, let's first save these changes to the Git
history. Let's open the Git interface with either the Git icon at the top near
the menu bar, or with `Ctrl-Alt-M`. When this opens up we'll click the checkbox
beside the `.gitignore` and `data-raw/mmash.R` files. Then we write a commit
message in the text box on the right, something like "Code to download data zip
file". Click the "Commit" button and close the Git interface.

Next, let's delete one of the files we don't need by using the [fs] package.
