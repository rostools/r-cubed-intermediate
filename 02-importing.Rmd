...
Our ultimate goal is to eventually save a working version of the raw data.
...
whenever you open this R Markdown file and running code in it. It's in this `setup`
code chunk that we will add `library()` functions when we want to load other
packages.
...
This shows that a specification is list and description of which columns are imported
into R and what data type they are given. For instance, `col_double()` means 
...
But... we also have a new message. You may or may not see this message
depending on the version of your packages (the most updated packages
will show this). It should say:
...
would add `.name_repair = snakecase::to_snake_case` to code.
...
```{r solution-import-the-saliva-data, include=FALSE, echo=TRUE, solution=TRUE}
...
```{r solution-import-the-actigraph-data, include=FALSE, echo=TRUE, solution=TRUE}
