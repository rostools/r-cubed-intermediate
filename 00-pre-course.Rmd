...
planning a full day to complete them.
...
Otherwise, if you can, come to the course earlier (15-20 min) to get help. <--**will 15 min be sufficient? Perhaps we should write 30 min ***
...
sleep and activity (MMASH) [@Rossi2020] <--*maybe we could link directly to PubMed or remove the direct link to the Refs?**. Look over these tasks and than switch 
...
To see the structure and content of a directory, we'll use the [fs] package, 
which means filesystem, and run the following code

```{r, eval=FALSE}
# To print the file list.
library(fs)
dir_tree("~/Desktop/LearnR3", recurse = 1)
```
...
package, which means filesystem, to work with files. Before you change any files,
